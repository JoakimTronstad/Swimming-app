...
sc_points = input("Insert points to be converted --> ")
try:
    int(sc_points) 
    it_is = True
except ValueError:
    it_is = False
while it_is == False:
    sc_points = input("Insert points to be converted in the form of an integer --> ")
    try:
        int(sc_points) 
        it_is = True
    except ValueError:
...
sc_time = float(sc_basetime/(int(sc_points)/1000)**(1/3))

#Convert time from seconds-format to minute-format
sc_minutes = int(int(sc_time)/60)

sc_time -= sc_minutes * 60

sc_seconds = int(sc_time)

sc_time -= sc_seconds

sc_time = sc_time * 100

sc_hundreds = int(sc_time)

sc_hundreds = sc_hundreds/100 

sc_seconds += sc_hundreds

#Print output
if sc_minutes > 0:
    print("Your time is " + str(sc_minutes) + "," + str(sc_seconds))
else:
    print("Your time is " + str(sc_seconds))
