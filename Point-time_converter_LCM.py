...
#Prompt user to insert points to be converted
lc_points = input("Insert points to be converted --> ")
try:
    int(lc_points) 
    it_is = True
except ValueError:
    it_is = False
while it_is == False:
    lc_points = input("Insert points to be converted in the form of an integer --> ")
    try:
        int(lc_points) 
        it_is = True
    except ValueError:
...
lc_time = float(lc_basetime/(int(lc_points)/1000)**(1/3))

#Convert time from seconds-format to minute-format
lc_minutes = int(int(lc_time)/60)

lc_time -= lc_minutes * 60

lc_seconds = int(lc_time)

lc_time -= lc_seconds

lc_time = lc_time * 100

lc_hundreds = int(lc_time)

lc_hundreds = lc_hundreds/100 

lc_seconds += lc_hundreds

#Print output
if lc_minutes > 0:
    print("Your time is " + str(lc_minutes) + "," + str(lc_seconds))
else:
    print("Your time is " + str(lc_seconds))
